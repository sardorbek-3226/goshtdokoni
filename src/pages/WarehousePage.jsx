...
export default function WarehousePage() {
  ...
  const loadData = async () => {
    try {
      ...
      const merged = apiData.map((p) => {
        const stockEntry = localStock.find(
          (s) => String(s.productId || s.id) === String(p.id)
        );
  ...
  const handleAddStock = (e) => {
    ...
    let localStock = JSON.parse(
      localStorage.getItem("warehouse_backup") || "[]"
    );

    const idx = localStock.findIndex(
      (s) => String(s.productId || s.id) === String(form.productId)
    );

    if (idx > -1) {
      localStock[idx].currentStock =
        Number(localStock[idx].currentStock || 0) + weightNum;
    } else {
  ...
  const totalStockValue = filtered.reduce((sum, p) => {
    return sum + Number(p.currentStock || 0) * Number(p.cost || 0);
  }, 0);
              ...
              <table className="w-full text-left border-collapse">
                <thead className="bg-slate-50/50">
                  <tr className="text-[10px] uppercase text-slate-400 font-black">
                    ...
                    <th className="p-6 text-right">Zaxira Qiymati</th>
                  </tr>
                </thead>
                ...
                <tbody className="divide-y divide-slate-50">
                  {loading ? (
                    ...
                    <tr>
                      <td
                        colSpan="4"
                        className="p-20 text-center text-slate-300 uppercase text-xs font-black italic"
                      >
                        Mahsulot topilmadi
                      </td>
                    </tr>
                    ...
                    filtered.map((p) => {
                      const stockValue =
                        Number(p.currentStock || 0) * Number(p.cost || 0);
                      ...
                      return (
                        ...
                        >
                          ...
                          <td className="p-6 text-center">
                            <div className="text-[9px] text-rose-400 line-through">
                              T: {Number(p.cost).toLocaleString()} UZS
                            </div>
                          ...
                          <td className="p-6 text-right">
                            <div
                              className={`inline-block px-5 py-2 rounded-2xl font-black text-xs ${
                                p.currentStock > 0
                                  ? "bg-emerald-50 text-emerald-600"
                                  : "bg-red-50 text-red-500"
                              }`}
                            >
                              {Number(p.currentStock || 0).toFixed(1)} KG
                            </div>
                          </td>
                ...
                {!loading && filtered.length > 0 && (
                  <tfoot className="bg-slate-900">
                    <tr>
                      <td
                        colSpan="3"
                        className="p-6 text-white font-black uppercase text-xs"
                      >
                        Jami zaxira qiymati
                      </td>
