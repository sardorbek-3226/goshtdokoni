...
export default function ProfilePage() {
  ...
  // Page ochilganda localStorage dan yuklab oladi
  useEffect(() => {
      ...
      <div className="relative bg-white p-8 rounded-[3rem] border border-slate-100 shadow-sm overflow-hidden">
        ...
        <div className="relative flex flex-col md:flex-row items-center gap-8">
          <div className="relative group">
            ...
            <button
              type="button"
              className="absolute bottom-1 right-1 p-2 bg-violet-600 text-white rounded-xl shadow-lg hover:bg-violet-700 transition-all"
            >
              <Camera size={18} />
            </button>
          </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
        <div className="space-y-6">
          <div className="bg-white p-6 rounded-[2.5rem] border border-slate-100 shadow-sm space-y-4">
            <h3 className="text-sm font-black text-slate-800 uppercase tracking-tighter mb-4">
              Xavfsizlik
            </h3>
            ...
            <button
              type="button"
              onClick={handleEdit}
              className="w-full flex items-center gap-3 px-4 py-3 bg-amber-50 text-amber-600 rounded-2xl text-xs font-bold hover:bg-amber-100 transition-all"
            >
              <User size={16} /> Tahrirlash
            </button>
        ...
        <div className="md:col-span-2">
          ...
          >
            <h3 className="text-lg font-black text-slate-800 italic mb-2">
              Shaxsiy ma'lumotlar
            </h3>
              ...
              <div className="space-y-2">
                ...
                <div className="relative">
                  ...
                  <input
                    ...
                    disabled={!isEditing}
                    className="w-full bg-slate-50 border-2 border-slate-100 rounded-2xl py-3.5 pl-12 pr-4 font-bold text-slate-700 focus:border-violet-500 outline-none transition-all disabled:opacity-70 disabled:cursor-not-allowed"
                  />
                </div>
              </div>
            ...
            <div className="pt-4 flex flex-wrap gap-3">
              <button
                type="submit"
                disabled={!isEditing}
                className="flex items-center justify-center gap-2 w-full sm:w-auto px-10 py-4 bg-slate-900 text-white rounded-2xl font-black text-xs uppercase tracking-widest hover:bg-violet-600 transition-all shadow-lg shadow-slate-200 disabled:opacity-50 disabled:cursor-not-allowed"
              >
              ...
              {isEditing && (
                <button
                  ...
                  className="w-full sm:w-auto px-8 py-4 bg-slate-100 text-slate-700 rounded-2xl font-black text-xs uppercase tracking-widest hover:bg-slate-200 transition-all"
                >
                  Bekor qilish
                </button>
              )}
