...
export default function DebtsPage() {
  ...
  const handleAddNewDebt = (e) => {
    ...
    if (!amount || amount <= 0) {
      return toast.error("Qarz miqdorini to‘g‘ri kiriting!");
    }
    ...
    const debtEntry = {
      id: Date.now(),
      customerName: newDebt.name.trim(),
    ...
    localStorage.setItem(
      "debts",
      JSON.stringify([...currentDebts, debtEntry])
    );
  ...
  const handlePayment = () => {
    ...
    if (!amount || amount <= 0) {
      return toast.error("To‘lov miqdorini kiriting!");
    }

    const allDebts = JSON.parse(localStorage.getItem("debts") || "[]");

    const updatedDebts = allDebts
      .map((d) => {
        if (String(d.id) !== String(selectedDebt.id)) return d;

        const oldRemaining = Number(d.remainingDebt || d.debt || 0);
      ...
      .filter((d) => Number(d.remainingDebt || 0) > 0);
    ...
    paymentHistory.push({
      id: Date.now(),
      customerName:
        selectedDebt.customerName || selectedDebt.name || "Nomsiz mijoz",
      amount,
      date: new Date().toISOString(),
    });
  ...
  const totalDebt = debts.reduce(
    (s, d) => s + Number(d.remainingDebt || d.debt || 0),
    0
  );
      ...
      <div className="max-w-6xl mx-auto space-y-8">
        ...
        <div className="grid grid-cols-1 gap-4">
          {filteredDebts.map((debt) => {
            const name = debt.customerName || debt.name || "Nomsiz mijoz";
            const amount = Number(debt.remainingDebt || debt.debt || 0);
          ...
          <div className="bg-white w-full max-w-md rounded-[3rem] p-10 shadow-2xl">
            ...
            <form onSubmit={handleAddNewDebt} className="space-y-5">
              <div className="space-y-2">
                <label className="text-[10px] uppercase text-slate-400 ml-2">
                  Mijoz Ismi
                </label>
                <input
                  type="text"
              ...
              <button
                type="submit"
                className="w-full bg-slate-900 text-white py-5 rounded-3xl font-black uppercase tracking-widest text-[10px] shadow-xl hover:bg-emerald-600 transition-all"
              >
                Ro‘yxatga qo‘shish
              </button>
            </form>
          ...
          <div className="bg-white w-full max-w-sm rounded-[3rem] p-8 shadow-2xl">
            ...
            <div className="bg-rose-50 p-6 rounded-[2rem] text-center mb-6">
              ...
              <p className="text-2xl font-black text-rose-600">
                {Number(
                  selectedDebt.remainingDebt || selectedDebt.debt || 0
                ).toLocaleString()}{" "}
                UZS
              </p>
            </div>
